...
interface EquipamentoDetalhe {
  ...
  // Terraplenagem: boa parte do dia real é máquina esperando abastecimento/
  // prancha, em manutenção corretiva ou se deslocando entre frentes — não é
  // "produção zero", é uma situação diferente que precisa aparecer no RDO.
  status: string;
...
function ehAtividadeIntocada(atividade: AtividadeDraft): boolean {
  return (
    ...
    atividade.largurasExtras.length === 0 &&
      ...
      {valores.map((valor, indice) => (
        <div key={indice} style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 4 }}>
          <input
            ...
            placeholder={`Largura extra ${indice + 1} (m)`}
            value={valor}
            onChange={(event) => {
              const novos = [...valores];
          ...
          setEquipamentosDetalheAberto(
            Object.fromEntries(
              rdo.equipamentos
                .filter(
                  (eq) =>
                    eq.producaoDescricao ||
                    eq.producaoValor != null ||
                    eq.horimetroInicial != null ||
                    eq.horimetroFinal != null ||
